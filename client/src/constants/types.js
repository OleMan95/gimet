export const NEW_EXPERT = 'NEW_EXPERT';
export const GET_HOME_BODY = 'GET_HOME_BODY';
export const GET_CONFIG_BODY = 'GET_CONFIG_BODY';
export const ADD_QUESTION = 'ADD_QUESTION';
export const GET_CONFIG_QUESTIONS = 'GET_CONFIG_QUESTIONS';
export const UPDATE_EXPERT = 'UPDATE_EXPERT';
export const SET_CONSULTATION_CONTENT = 'SET_CONSULTATON_CONTENT';
export const SET_CONSULTATION_EXPERT = 'SET_CONSULTATION_EXPERT';
export const FILTER_EXPERTS = 'FILTER_EXPERTS';
